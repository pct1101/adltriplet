...
class ControllerAdminAPI extends Controller
{

    //Lấy danh sách xe, hãng xe, users, xe yêu thích(favorite), bình luận (feedback)
    public function dashboardadmin()
    {
        $car = car::orderBy('car_id', 'desc')->paginate(10);
        $brands = carbrand::all();
        $DataOfCar = CarResources::collection($car);
        $DataOfBrands = CarbrandResource::collection($brands);
        $DataOfUsers = CarbrandResource::collection($brands);
        $DataOfFavorite = CarbrandResource::collection($brands);
        $DataOfFeedback = CarbrandResource::collection($brands);
        return response()->json([$DataOfCar, $DataOfBrands, $DataOfUsers, $DataOfFavorite, $DataOfFeedback]);
